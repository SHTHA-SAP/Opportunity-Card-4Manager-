...
def map_type(cap: dict, product_group: dict = None) -> str:
    ...
    Checks BOTH the capability-level AND the product-group-level fields,
    because SAP may store commercialType at either level.
    
    Never guesses — reads directly from the API data.
    """
    # Build the list of dicts to check (capability first, then parent group)
    sources = [cap]
    if product_group and product_group is not cap:
        sources.append(product_group)

    # All possible field names that might contain the commercial type
    type_fields = [
        ...
        "pricingType", "PricingType", "pricing_type",
        "tier", "Tier",
        "ribbons", "Ribbons",
...
def map_icon(name: str, ribbons: str, cap: dict = None, product_group: dict = None) -> str:
    """
    Determine the icon type from API data.
    Checks the aiType field first (most reliable), then falls back to name matching.
    """
    # Check aiType field (SAP may use this to categorize)
    sources = []
    if cap:
        sources.append(cap)
    if product_group and product_group is not cap:
    ...
    for source in sources:
        for field in ["aiType", "AiType", "ai_type", "type", "Type"]:
            val = (source.get(field) or "").lower()
            if "agent" in val:
                return "agents"
            if "joule" in val:
                return "joule"

    # Fallback: check name and ribbons text
    n = name.lower()
    r = (ribbons or "").lower()
    if "agent" in n:
        return "agents"
...
def build_catalog(raw: list) -> list:
    ...
    Passes BOTH the capability AND the product group to map_type/map_icon
    so they can read the commercial type from whichever level has it.
    """
    catalog = []
    seen = set()

    # ── Comprehensive DEBUG: dump full structure so we can verify field mapping ──
    if raw:
        first = raw[0]
        print(f"\n{'─'*60}")
        ...
        # Show all non-list, non-dict values at the product group level
        for k, v in first.items():
            if not isinstance(v, (list, dict)):
                print(f"    {k}: {repr(v)}")
        caps = first.get("capabilities", [])
        if caps and isinstance(caps[0], dict):
            print(f"\n  Capability fields: {sorted(caps[0].keys())}")
            # Show all non-list, non-dict values at capability level
            for k, v in caps[0].items():
                if not isinstance(v, (list, dict)):
                    print(f"    {k}: {repr(v)}")
        # Show second product group too (might have different type)
        if len(raw) > 1:
            second = raw[1]
    ...
    for product_group in raw:
        ...
        if not isinstance(capabilities, list):
            # Maybe this item IS a capability (flat structure, no nesting)
            if "name" in product_group and ("shortDescription" in product_group or "description" in product_group):
                ...
                if key not in seen:
                    ...
                    catalog.append({
                        ...
                        "desc": desc,
                        "type": map_type(cap, None),
                        "icon": map_icon(name, ribbons, cap, None),
                    })
            continue
        ...
        for cap in capabilities:
            ...
            catalog.append({
                ...
                "desc": desc,
                "type": map_type(cap, product_group),
                "icon": map_icon(name, ribbons, cap, product_group),
            })
