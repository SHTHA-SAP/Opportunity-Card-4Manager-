...
def map_type(item):
    ...
    Priority order:
      1. Ribbons tier tags (primary source of truth):
           "EAC"         -> premium (cyan)  — Enterprise Add-on Content
           "premium_*"   -> premium (cyan)  — e.g. premium_concur
           "Base"        -> base (lavender)
      2. Package metadata (for ai_feature_package items):
           If packageMetadata exists -> premium
           (The Discovery Center website shows these as "Premium" but
            the ribbons field doesn't always include a tier tag.)
      3. Fallback -> base
    ...
    """
    # Check ribbons for tier signals
    ...
    # Package items (ai_feature_package, ai_agent_package, etc.) with
    # packageMetadata are part of premium Joule packages — the website
    # shows them as "Premium" even when ribbons lack a tier tag.
