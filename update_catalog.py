...
def map_type(item):
    """Map API type to card type (base/premium).
    - ai_agent / ai_agent_package -> premium (cyan cards)
    - ai_feature / ai_feature_package -> base (lavender cards)
    - ai_assistant / ai_assistant_package -> base (lavender cards)
    Falls back to isBillable if type is unrecognised.
    """
    api_type = _normalize_api_type(item)
    if api_type in ("ai_agent", "ai_agent_package"):
        return "premium"
    if api_type in ("ai_feature", "ai_feature_package"):
        return "base"
