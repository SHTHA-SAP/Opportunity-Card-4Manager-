...
# Separate calls for features, agents, and assistants
API_URLS = [
    API_BASE + "?type=ai-feature",
    API_BASE + "?type=ai-agent",
    API_BASE + "?type=ai-assistant",
]
...
def _normalize_api_type(item):
    """Normalize the API type field — handles both hyphens and underscores."""
    return (item.get("type") or "").lower().strip().replace("-", "_")


def map_type(item):
    """Map to card type (base/premium) using API data fields.

    The Discovery Center website shows the "Premium" tag based on
    multiple signals.  We check them in priority order:

      1. isBillable flag  — if true the item requires AI Units or
         an additional license  -> premium (cyan).
      2. Ribbons tier tags:
           "EAC"         -> premium  (Enterprise Add-on Content,
                            sometimes isBillable=false for EAC items)
           "premium_*"   -> premium  (e.g. premium_concur)
      3. packageMetadata — present on ai_feature_package items that
         belong to premium Joule packages.
      4. Fallback        -> base (lavender).

    Capability tags in ribbons are ignored for tier:
        "GenAI", "Narrow AI", "Joule", "Featured", "Beta", "New", "Promotion"

    No API-type overrides — data fields are the source of truth so that
    weekly auto-updates always reflect whatever SAP publishes.
    """
    # 1. isBillable is the primary premium signal
    if item.get("isBillable"):
        return "premium"

    # 2. Check ribbons for tier signals (catches EAC items where isBillable=false)
    ribbons = (item.get("ribbons") or "").lower()
    ribbon_list = [r.strip() for r in ribbons.split(",") if r.strip()]

    for r in ribbon_list:
        if "premium" in r:
            return "premium"
        if r == "eac":
            return "premium"

    # 3. Package items belong to premium Joule packages
    if item.get("packageMetadata"):
        return "premium"
...
def map_icon(item):
    ...
    api_type = _normalize_api_type(item)
    name = (item.get("name") or "").lower()
    ribbons = (item.get("ribbons") or "").lower()

    if api_type in ("ai_agent", "ai_agent_package"):
        return "agents"
    if api_type in ("ai_assistant", "ai_assistant_package"):
        return "joule"
    if "joule" in name:
        return "joule"
    if "joule" in ribbons:
...
async def fetch_catalog():
    ...
    async with async_playwright() as p:
        ...
        # Map each API URL to the correct Referer (matches browser behaviour)
        REFERER_MAP = {
            "ai-feature":   "https://discovery-center.cloud.sap/ai-features/",
            "ai-agent":     "https://discovery-center.cloud.sap/ai-agents/",
            "ai-assistant": "https://discovery-center.cloud.sap/ai-assistants/",
        }

        for url in API_URLS:
            print(f"\n  Fetching: {url}")
            # Pick correct referer based on the type= query param
            referer = "https://discovery-center.cloud.sap/"
...
def build_catalog(raw):
    ...
    # Whitelist: only these API types make it into the catalog (both underscore and hyphen forms)
    ALLOWED_TYPES = {
        "ai_feature", "ai_feature_package", "ai-feature", "ai-feature-package",
        "ai_agent", "ai_agent_package", "ai-agent", "ai-agent-package",
        "ai_assistant", "ai_assistant_package", "ai-assistant", "ai-assistant-package",
    }
    skipped_types = {}
    ...
    for item in raw:
        ...
        # Only keep allowed types — skip unknown types
    ...
    # Debug: show premium vs base split in output
    p = sum(1 for c in catalog if c["type"] == "premium")
    b = sum(1 for c in catalog if c["type"] == "base")
    print(f"  Output split: {p} premium, {b} base")
    if catalog:
        for entry in catalog[:3]:
            print(f"    sample: {entry['name']!r} -> type={entry['type']}, icon={entry['icon']}")
