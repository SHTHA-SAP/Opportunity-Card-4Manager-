...
def map_type(item):
    """Map to card type (base/premium) using the ribbons field.

    The `ribbons` field is a comma-separated string with tier + capability tags:
      Tier tags:   "Base"  -> base (lavender)
                   "EAC"   -> premium (cyan)  — Enterprise Add-on Content
                   "premium_*" -> premium (cyan)
      Capability:  "GenAI", "Narrow AI", "Joule" — ignored for tier

    For ai_agent types, always premium.
    Falls back to base if nothing matches.
    """
    api_type = _normalize_api_type(item)

    # Agents are always premium
