...
def map_type(cap: dict) -> str:
    candidates = [
        cap.get("commercialType"), cap.get("CommercialType"),
        cap.get("package"), cap.get("Package"),
        cap.get("licenseModel"), cap.get("LicenseModel"),
        cap.get("packageName"), cap.get("ribbons"), cap.get("tier"),
    ]
...
async def try_api_intercept(page) -> list:
    ...
    # Try clicking load-more / cookie consent buttons
    for selector in ['button:has-text("Load More")', 'button:has-text("Show All")',
                     'button:has-text("Show more")', 'button:has-text("Accept")',
                     'button:has-text("Accept All")', '[class*="showMore"]']:
        try:
...
async def fetch_catalog() -> list:
    ...
    async with async_playwright() as p:
        ...
        for url in PAGE_URLS:
            ...
            # Strategy 2: DOM scraping (only if API failed)
            if not result:
                dom_data = await try_dom_scraping(page)
                if dom_data:
                    result.extend(dom_data)
...
def build_catalog(raw: list) -> list:
    ...
    if raw:
        print(f"\nDEBUG: First item keys: {list(raw[0].keys())}")
        ...
        if isinstance(capabilities, list) and capabilities:
            ...
            for cap in capabilities:
                if not isinstance(cap, dict):
                    continue
                entry = _extract_entry(cap, product_name)
...
def _extract_entry(cap: dict, product_name: str):
